...
class ModelManager {
    ...
    public function __construct($db, $aSetting)
    {
		...
		public function exeApi($obj){
			$q  = "select m.id_model, m.model_name, m.id_crop, c.crop_name, m.id_pest, p.pest_name, m.model_description from model m left join crop c on c.id_crop = m.id_crop left join pest p on p.id_pest = m.id_pest";
